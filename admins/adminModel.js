...
if(id){
    const data= await db(name).where('id',id).first();
    return data;
}
 return await db(name)
} 
const deleted = async(name, id) =>{
    return await db(name).where('id',id).del()
}
...
const getAllusers =async(id) => {
    ...
    .select('us.firstname','us.lastname','tr.no_of_kids','fl.departure_time','fl.arrival_time','arr.user_location')
